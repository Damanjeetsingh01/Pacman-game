...
class Display:
    ...
    def show_score(self, score, screen):
        ...
        screen.blit(
            text,
            ((self.x + WIDTH) - (7 * self.size), self.y)
        )
    ...
    def show_timer(self, screen, text):
        ...
        for i in range(3, 0, -1):
            timer_text = self.font.render(
                f"The game will start in {i}",
                True,
                (255, 255, 255)
            )
            main_text = self.font.render(
                f"{text}",
                True,
                (255, 255, 255)
            )
    ...
    def show_menu(
        self, screen, close_menu, change_background_color,
        background_color, change_walls_color, walls_color
    ):
        background_rect = pygame.Rect(0, 0, WIDTH, HEIGHT)
        pygame.draw.rect(screen, (0, 0, 0), background_rect)

        play_button = Button(
            "Play", WIDTH // 2 - 135, HEIGHT // 2 - 50, 270, 50
        )
        change_walls_color_button = Button(
            "Change wall color", WIDTH // 2 - 135, HEIGHT // 2 + 20, 270, 50
        )
        change_background_color_button = Button(
            "Change background color", WIDTH // 2 - 135, HEIGHT // 2 + 90, 270, 50
        )
