...
class Pacman(pygame.sprite.Sprite):
    ...
    def is_collide(self, x, y, walls_collide_list):
        ...
        return rect.collidelist(walls_collide_list) != -1
    ...
    def rotate_pacman(self):
        if self.direction_name == "right":
            ...
            self.image = pygame.transform.rotate(
                self.frames[self.frame_index], 180
            )
        elif self.direction_name == "up":
            self.image = pygame.transform.rotate(
                self.frames[self.frame_index], 90
            )
    ...
    def move(self, pressed_key, walls_collide_list):
        for key, value in self.keys.items():
            if (pressed_key[value] and
                    not self.is_collide(
                        *self.directions[key], walls_collide_list
                    )):
